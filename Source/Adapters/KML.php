...
class KML extends GeoAdapter
{
    ...
    protected function parsePoint($xml)
    {
        ...
        {
            $point = new Point($coordinates[0][0], $coordinates[0][1]);
            // set the point name
            $nameElements = $xml->parentNode->getElementsByTagName('name');
            $point->name = $nameElements->length > 0 ? $nameElements->item(0)->nodeValue : null;
