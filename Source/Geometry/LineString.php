...
class LineString extends Collection
{
	...
	public function preciseLength()
	{
		...
		for ($i = 0; $i < $this->numPoints() - 1; $i++)
		{
			...
			$lat = deg2rad($next_point->coords[0] - $point->coords[0]);
			$lon = deg2rad($next_point->coords[1] - $point->coords[1]);
			$a = sin($lat/2)*sin($lat/2) + cos(deg2rad($point->coords[0]))*cos(deg2rad($next_point->coords[0]))*sin($lon/2)*sin($lon/2);
			$c = 2*atan2(sqrt($a), sqrt(1 - $a));
			$distance = 6378137*$c;
			$height = $point->z() - $next_point->z();
